...
**목표:** Slack DM에서 "소재서치" 입력 또는 `/소재서치` 슬래시 커맨드 실행 시, Perplexity sonar-pro(웹 검색 내장)를 통해 C레벨 임원 대상 아티클 후보 7건을 DM으로 발송한다. 재생성·확정·시리즈 기획 버튼 포함.
...
- [🔄 재생성] 클릭 → 세션에 저장된 이전 키워드 목록을 프롬프트에 제외 조건으로 추가 → 새 소재 7건으로 메시지 교체(update)
...
- [📚 시리즈 기획 보기] 클릭 → 시리즈 기획 프롬프트로 Perplexity 호출 → 시리즈 7건을 별도 메시지로 DM에 추가 발송
- 시리즈도 드롭다운 + [확정] 버튼 동일하게 제공
...
### 모델
`perplexity/sonar-pro` via OpenRouter API
...
sessions = {
    "USER_ID": {
        ...
        "series": [...],          # 현재 시리즈 7건 (있을 경우)
        "used_keywords": [...],   # 이전 키워드 누적 (재생성 중복 방지)
        "last_ts": "...",         # 마지막 메시지 타임스탬프 (update용)
        "last_channel": "...",
    }
}
```
